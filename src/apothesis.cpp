...
void Apothesis::init()
{
    ...
    params.insert( {"T", 623.} );
    params.insert( {"P", 1333.} );
    params.insert( {"f", 4.0e-5} );
    params.insert( {"C_tot", 2.0e+19} ); // For Cu (from
    params.insert( {"s0", 0.09} );
...
void Apothesis::exec()
{
    ...
    double writeLatHeigsEvery = 0.01; //in s
...
//                cout << " m_dProcRate " <<  m_dProcRate << "m_dRTot " << m_dRTot << endl;
...
//                            if ( p2.second.empty() )
 //                               cout << p2.first->getName() <<  " It is empty " << endl;
            ...
            if ( timeToWrite >= writeLatHeigsEvery ) {
                ...
                output = std::to_string(m_dProcTime) + '\t' + std::to_string( fabs(pProperties->getMeanDH() - aveDH1)/m_dProcTime ) + '\t' + std::to_string( pProperties->getRMS() )  + '\t' ;
                for ( auto &p:m_processMap)
                    output += std::to_string( p.first->getNumEventHappened() ) + '\t';
