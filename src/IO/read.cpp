...
string Read::getInputPath() const {;}

void Read::readInputFile()
  {
...
void Read::openInputFile(string file)
  {
...
string Read::simplified( string str )
  {
  string s;
  char c;
  bool is_white = false;
...
vector<string> Read::split( string str, string delim ){
  vector<string> r;
  size_t prev = 0, pos = 0;
  do {
...
double Read::toDouble( string str )
  {
  if ( isNumber( str ) )
    return stod( str);
  else
...
bool Read::contains(string str1, string str2, CASE cas )
  {
  switch (cas) {
    case Insensitive:{
      char c;
...
bool Read::openOutputFile( string name )
  {
...
void Read::closeOutputFile()
  {
  if ( m_OutFile.is_open( ) )
    m_OutFile.close();
  }

void Read::writeLogOutput( string str )
  {
    m_OutFile << str << endl;
  }

void Read::writeLatticeInfo()
  {
...
string Read::GetCurrentWorkingDir()
  {
...
Lattice::Type Read::getLatticeType()
  {
