...
{      
    ...
    //            cout << pProperties->getMeanDH()  <<  " " << meanDHPrevStep <<  " " << m_dProcTime << " " << prevTimeStep << " " <<  pProperties->getMeanDH() - meanDHPrevStep << endl;
                ...
                // output += "Inside computation" + '\t';
                // pIO->writeInOutput( output );
                //Send Message instead
                MPI_Request request;
                MPI_Isend(output.c_str(), output.size() + 1, MPI_CHAR, 0, LOG_TAG, MPI_COMM_WORLD, &request);
                // MPI_Send(output.c_str(), output.size() + 1, MPI_CHAR, 0, LOG_TAG, MPI_COMM_WORLD);
                timeToWriteLog = 0.0;
            }
        ...
        pIO->writeInOutput( output );
        // MPI_Send(output.c_str(), output.size() + 1, MPI_CHAR, 0, LOG_TAG, MPI_COMM_WORLD);
        MPI_Send(nullptr, 0, MPI_CHAR, 0, EXIT_TAG, MPI_COMM_WORLD);
        if ( m_bHasGrowth )
            pIO->writeLatticeHeights( m_dProcTime );
        ...
        while(1){
            char buffer[512];
            MPI_Status status;
            MPI_Recv(buffer, 512, MPI_CHAR, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);

            if (status.MPI_TAG == LOG_TAG) {
